...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			try {
				//BeanFactory准备工作完成后进行的后置处理工作(扩展点)
				//上面对beanFactory进行了许多配置，现在需要对beanFactory进行一些处理。不同的实现做不同的操作。
				...
				//初始化MessageSource组件（主要是国际化功能,不重要）；
				initMessageSource();
	...
	protected void prepareRefresh() {
		...
		// 扩展点：留给子类覆写,初始化context environment中的占位符属性的处理
		initPropertySources();

		// 创建并获取环境对象，验证需要的属性文件是否都已经放入环境中 参见ConfigurablePropertyResolver#setRequiredProperties
	...
	protected ConfigurableListableBeanFactory obtainFreshBeanFactory() {
		...
		//AbstractRefreshableApplicationContext（重新创建一个bean工厂，并设置工厂的一些属性）
		//GenericApplicationContext（获取创建容器的就创建的bean工厂，并且设置工厂的ID.）
		refreshBeanFactory();
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		//设置一些必要的自定义属性编辑注册器 查看ResourceEditorRegistrar#registerCustomEditors方法
		...
		//设置忽略的自动装配的接口六个Aware
		//ApplicationContextAwareProcessor#invokeAwareInterfaces已经把这5个接口的实现工作做了
	...
	/**
	 * 先判断有没有自定义的ApplicationEventMulticaster，没有的话就注册一个。
	 * SimpleApplicationEventMulticaster就是用来发布事件用的。
	 * @see org.springframework.context.event.SimpleApplicationEventMulticaster
	 */
	...
	protected void registerListeners() {
		...
		for (ApplicationListener<?> listener : getApplicationListeners()) {
			// 把listener放入applicationListeners中
		...
		// 从容器中获取所有实现了ApplicationListener接口的beandefinition名称
		String[] listenerBeanNames = getBeanNamesForType(ApplicationListener.class, true, false);
		for (String listenerBeanName : listenerBeanNames) {
			//名称放入applicationListenerBeans
	...
	protected void finishRefresh() {
		...
		// getLifecycleProcessor()会放入上一行的初始化的LifecycleProcessor对象
		...
		//TODO 目前没用到，初步阅读是管理所有已注册的ApplicationContext
